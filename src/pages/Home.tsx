    ...
    return (
        <div>
            <div className="flex items-center justify-around p-1">
                <img src={cadusLogo} className="h-64 w-64 bg-white home-logo" alt="Logo Cadus"/>
                <div className="text-center w-1/2 ">
                    <h3 className="text-2xl font-bold">
                        Vous etes victime d'erreurs médicales ?
                    </h3>
                    <p className="text-lg mt-2">
                        L'assistance de la C.A.D.U.S conseil les victime dans les actions contentieuses pour l'obtention
                        de l'indemnisation des dommages subis.
                    </p>
                </div>
            </div>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-2 lg:grid-cols-3 gap-8 items-center mt-8 p-8">
                {aideList.map((item, index) => (
                    <div key={index} className="cases w-96 h-32 bg-white shadow-md rounded-lg flex text-center items-center p-6">
                        <p>{item}</p>
                    </div>
                ))}
            </div>
